...
with st.sidebar:
    ...
    #         client_df= pd.read_csv(client_data_file, index_col=False)
    #     price_data_file = st.file_uploader("Upload Price CSV data file", type=["csv"])
    #     if price_data_file is not None:
    #         price_df = pd.read_csv(price_data_file, index_col=False)
    
    # with st.expander('Exploratory Data Analysis'):
    #     # density_plot(consumption)
    #     # Add a button to navigate to EDA
    #     if st.button("Explore Data"):
    #         st.session_state.show_eda = not st.session_state.get("show_eda", False)
    #         st.experimental_rerun()  # Rerun to update the UI based on session state

    #     # Check if show_eda is True to display EDA section
    #     if st.session_state.get("show_eda", False):
    #         st.write("EDA Section Placeholder")
    #         density_plot(consumption)
...
- price_mid_peak_var, price_peak_fix and price_mid_peak_fix have a median of zero. This suggests that at least half of the observations are zero which indicates a skewness towards zero.''')
    ...
    # plot consumption in 2015 and compare to price variability to see that price isn't sensitive to consumption
    # plt.figure(figsize=(15,5))
